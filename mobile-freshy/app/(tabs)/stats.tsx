...
function WastedList({ items }: { items: { emoji: string; name: string; brand: string; veces: number }[] }) {
  ...
  return (
    <View style={styles.section}>
      ...
      {items.map((item, idx) => (
        <View key={item.name} style={styles.wastedRow}>
          <Text style={styles.wastedRank}>{idx + 1}</Text>
          <Text style={styles.wastedEmoji}>{item.emoji}</Text>
          <View style={styles.wastedInfo}>
            <Text style={styles.wastedName}>{item.name}</Text>
            <Text style={styles.wastedBrand}>{item.brand}</Text>
            <View style={styles.wastedTrack}>
              <View style={[styles.wastedFill, { width: `${(item.veces / max) * 100}%` as any }]} />
            </View>
          </View>
          <View style={styles.wastedBadge}>
            <Text style={styles.wastedVeces}>{item.veces}</Text>
            <Text style={styles.wastedVecesLabel}>{item.veces === 1 ? 'vez' : 'veces'}</Text>
          </View>
        </View>
      ))}
    </View>
    ...
    <View style={styles.section}>
      ...
      {items.map((item, idx) => (
        <View key={item.name} style={styles.wastedRow}>
          ...
          <View style={styles.wastedBadge}>
            <Text style={styles.fastVeces}>{item.promedioDias}d</Text>
            <Text style={styles.wastedVecesLabel}>promedio</Text>
          </View>
        </View>
      ))}
    </View>
